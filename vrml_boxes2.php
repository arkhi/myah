...
$select_IP = mysql_query("SELECT * FROM myah GROUP BY pseudo ORDER BY pseudo") or die("Impossible d'effectuer la requête\n <!-- ".mysql_error()." -->\n");
// print_r($tab_points);
...
Viewpoint {
	position		<?php print((-850*$scale_model)." ".(16*$scale_model)." ".(800*$scale_model)); ?>
	orientation		0 1 0 -1
	description		"vue 1"
	jump			TRUE
}
Viewpoint {
	position		<?php print((-100*$scale_model)." ".(16*$scale_model)." ".(1000*$scale_model)); ?>
...
Fog {
	color            1 1 1
	fogType          "EXPONENTIAL "
	visibilityRange  <?php print(7000*$scale_model); ?>
}

Transform {
	translation -4000 -0.5 4000
	children [
		Shape {
